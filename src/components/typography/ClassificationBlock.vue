<script setup></script>

<template>
  <div
    class="mr-[135px] lg:mr-[100px] md:mr-[50px] sm:mr-[20px] bg-customPurple rounded-r-[100px] h-[40px]"
  ></div>
  <div
    class="grid grid-cols-12 grid-rows-2 gap-y-[30px] gap-x-[10px] px-[135px] lg:px-[100px] md:px-[50px] sm:px-[20px] mb-20 xs:mb-0 mt-10"
  >
    <div
      class="col-start col-end-2 row-start-1 row-end-2 group aspect-square hover:bg-customOrange bg-customLime rounded-[60px] px-12 md:px-8 xs:px-6 py-2 flex items-center justify-center"
    >
      <span
        class="font-myUnbounded font-bold text-customBlack text-[55px] md:text-[45px] sm:text-[35px] xs:text-[20px] group-hover:text-white"
        >3</span
      >
    </div>
    <h2
      class="col-start-3 col-end-6 row-start-1 row-end-2 self-center font-myMedium text-white text-[52px] lg:text-[40px] md:text-[30px] sm:text-[20px] xs:text-[14px]"
    >
      классификация
    </h2>
    <p
      class="col-start-1 col-end-13 row-start-2 row-end-3 self-center font-myUnbounded font-light text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px]"
    >
      — шрифты, как и люди, бывают разными: с характером, с историей, с особенными чертами. Их
      классифицируют по стилю, характеру и начертанию, чтобы дизайнеры могли легко ориентироваться в
      их многообразии.
    </p>
  </div>

  <div
    class="grid grid-cols-12 grid-rows-12 lg:grid-rows-7 gap-y-[30px] gap-x-[10px] px-[135px] lg:px-[100px] md:px-[50px] sm:px-[20px] mb-40 xs:mb-20"
  >
    <h3
      class="text-customLime xs:self-end col-start-1 col-end-8 row-start-1 row-end-2 font-myUnbounded font-light text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] self-center"
    >
      Типы шрифтов:
    </h3>
    <!-- 1st font type -->
    <p
      class="col-start-1 col-end-8 row-start-2 row-end-3 text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] font-myUnbounded font-light self-center"
    >
      <span class="text-customOrange">Гротески: </span>современные шрифты без засечек. Чистые,
      простые и динамичные. Используются для веб-дизайна, современного брендинга.
    </p>
    <img
      class="col-start-8 col-end-9 row-start-2 row-end-3 self-center"
      src="/src/img/decoration_star_lime.svg"
      alt="Ошибка загрузки изображения! Попробуйте перезагрузить странцу"
    />
    <div
      class="col-start-9 col-end-13 row-start-2 row-end-3 hover:bg-opacity-10 border-4 border-customPurple bg-customPurple rounded-l-[60px] lg:rounded-l-[40px] md:rounded-l-[40px] sm:rounded-l-[30px] rounded-tr-[60px] lg:rounded-tr-[40px] md:rounded-tr-[40px] sm:rounded-tr-[30px] px-10 sm:px-4 xs:pr-2 py-4 lg:py-2 flex items-center justify-end text-end self-center"
    >
      <p class="text-white text-[30px] lg:text-[22px] md:text-[18px] font-myPhudu">Пример шрифта</p>
    </div>

    <!-- 2nd font block  -->
    <p
      class="col-start-1 col-end-8 row-start-4 row-end-5 lg:row-start-3 lg:row-end-4 text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] font-myUnbounded font-light self-center"
    >
      <span class="text-customOrange">Акцидентные: </span>шрифты с острыми углами, ярким характером,
      часто используются в рекламе и дизайне плакатов.
    </p>
    <div
      class="col-start-8 col-end-13 row-start-4 row-end-5 lg:row-start-3 lg:row-end-4 group hover:bg-opacity-10 border-4 border-customLime bg-customLime rounded-l-[60px] lg:rounded-l-[40px] md:rounded-l-[40px] sm:rounded-l-[30px] rounded-tr-[60px] lg:rounded-tr-[40px] md:rounded-tr-[40px] sm:rounded-tr-[30px] px-10 sm:px-4 xs:pr-2 py-4 lg:py-2 flex items-center justify-end text-end self-center"
    >
      <p
        class="text-customBlack text-[40px] lg:text-[30px] md:text-[25px] font-mySoledago group-hover:text-white"
      >
        Пример шрифта
      </p>
    </div>

    <!-- 3rd font type -->
    <p
      class="col-start-1 col-end-8 row-start-6 row-end-7 lg:row-start-4 lg:row-end-5 text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] font-myUnbounded font-light self-center"
    >
      <span class="text-customOrange">Текстовые: </span>шрифты, предназначенные для больших объемов
      текста. Они обычно имеют засечки и хорошую читаемость.
    </p>
    <img
      class="col-start-8 col-end-9 row-start-6 row-end-7 lg:row-start-4 lg:row-end-5 self-center"
      src="/src/img/decoration_star_lime.svg"
      alt="Ошибка загрузки изображения! Попробуйте перезагрузить странцу"
    />
    <div
      class="col-start-9 col-end-13 row-start-6 row-end-7 lg:row-start-4 lg:row-end-5 hover:bg-opacity-10 border-4 border-customPurple bg-customPurple rounded-l-[60px] lg:rounded-l-[40px] md:rounded-l-[40px] sm:rounded-l-[30px] rounded-tr-[60px] lg:rounded-tr-[40px] md:rounded-tr-[40px] sm:rounded-tr-[30px] px-10 sm:px-4 xs:pr-2 py-4 lg:py-2 flex items-center justify-end text-end self-center"
    >
      <p class="text-white text-[30px] lg:text-[22px] md:text-[18px] font-myAlice">Пример шрифта</p>
    </div>

    <!-- 4th font block  -->
    <p
      class="col-start-1 col-end-8 row-start-8 row-end-9 lg:row-start-5 lg:row-end-6 text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] font-myUnbounded font-light self-center"
    >
      <span class="text-customOrange">Антиквы: </span>классические шрифты с засечками, отличаются
      элегантностью и формальностью. Используются для книг, журналов, официальных документов.
    </p>
    <div
      class="col-start-8 col-end-13 row-start-8 row-end-9 lg:row-start-5 lg:row-end-6 group hover:bg-opacity-10 border-4 border-customLime bg-customLime rounded-l-[60px] lg:rounded-l-[40px] md:rounded-l-[40px] sm:rounded-l-[30px] rounded-tr-[60px] lg:rounded-tr-[40px] md:rounded-tr-[40px] sm:rounded-tr-[30px] px-10 sm:px-4 xs:pr-2 py-4 lg:py-2 flex items-center justify-end text-end self-center"
    >
      <p
        class="text-customBlack text-[30px] lg:text-[28px] md:text-[24px] font-myVollkornSC group-hover:text-white"
      >
        Пример шрифта
      </p>
    </div>

    <!-- 5th font type -->
    <p
      class="col-start-1 col-end-8 row-start-10 row-end-11 lg:row-start-6 lg:row-end-7 text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] font-myUnbounded font-light self-center"
    >
      <span class="text-customOrange">Брусковые: </span>шрифты с прямыми линиями и резкими
      переходами. Используются для создания строгого и контрастного визуального эффекта.
    </p>
    <img
      class="col-start-8 col-end-9 row-start-10 row-end-11 lg:row-start-6 lg:row-end-7 self-center"
      src="/src/img/decoration_star_lime.svg"
      alt="Ошибка загрузки изображения! Попробуйте перезагрузить странцу"
    />
    <div
      class="col-start-9 col-end-13 row-start-10 row-end-11 lg:row-start-6 lg:row-end-7 hover:bg-opacity-10 border-4 border-customPurple bg-customPurple rounded-l-[60px] lg:rounded-l-[40px] md:rounded-l-[40px] sm:rounded-l-[30px] rounded-tr-[60px] lg:rounded-tr-[40px] md:rounded-tr-[40px] sm:rounded-tr-[30px] px-10 sm:px-4 xs:pr-2 py-4 lg:py-2 flex items-center justify-end text-end self-center"
    >
      <p class="text-white text-[45px] lg:text-[35px] md:text-[30px] font-myBravoSC leading-8">
        Пример шрифта
      </p>
    </div>

    <!-- 6th font block  -->
    <p
      class="col-start-1 col-end-8 row-start-12 row-end-13 lg:row-start-7 lg:row-end-8 text-white text-[22px] lg:text-[18px] sm:text-[14px] xs:text-[12px] font-myUnbounded font-light self-center"
    >
      <span class="text-customOrange">Рукописные: </span>шрифты имитируют письмо от руки и таким
      образом персонализируют текст.
    </p>
    <div
      class="col-start-8 col-end-13 row-start-12 row-end-13 lg:row-start-7 lg:row-end-8 group hover:bg-opacity-10 border-4 border-customLime bg-customLime rounded-l-[60px] lg:rounded-l-[40px] md:rounded-l-[40px] sm:rounded-l-[30px] rounded-tr-[60px] lg:rounded-tr-[40px] md:rounded-tr-[40px] sm:rounded-tr-[30px] px-10 sm:px-4 xs:pr-2 py-4 lg:py-2 flex items-center justify-end text-end self-center"
    >
      <p
        class="text-customBlack text-[40px] lg:text-[30px] md:text-[25px] font-myStampatelloFaceto group-hover:text-white"
      >
        Пример шрифта
      </p>
    </div>
  </div>
</template>

<script>
export default {
  name: 'ClassificationBlock'
}
</script>

<style scoped></style>
